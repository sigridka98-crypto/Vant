...
function countTemplateWords(state: DraftState) {
  return [
    state.description,
    state.safeExample,
    state.quickMemoryRule,
    state.howItWorks,
    state.redFlags,
    state.protection
...
export function CardForm({ mode, action, card, error }: CardFormProps) {
  ...
  const templateWordCount = countTemplateWords(draft);
  const templateWordLimitReached = templateWordCount > MAX_TEMPLATE_WORDS;
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        if (templateWordLimitReached) {
          event.preventDefault();
          setInlineError("This lesson card is too long. Keep the full lesson template under 1,000,000 words.");
          return;
        }
    ...
    >
      ...
      <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="vant-card rounded-[28px] p-5">
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-text-secondary">One-card lesson writer</p>
          <div className="mt-4 space-y-3 text-sm leading-6 text-text-secondary">
            <p>Write the whole lesson template here as one complete card.</p>
            <p>The summary, safe example, quick memory rule, core sub-lessons, red flag sub-lessons, and protection sub-lessons all belong to the same lesson card.</p>
            <p>Keep it educational, clearly structured, and under 1,000,000 words for the full lesson template.</p>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-4">
            <LessonCounter label="Core sub-lessons" count={countLines(draft.howItWorks)} />
            <LessonCounter label="Red flags" count={countLines(draft.redFlags)} />
            <LessonCounter label="Protection" count={countLines(draft.protection)} />
            <LessonCounter label="Total template words" count={templateWordCount} />
          </div>
          <div className="mt-5 rounded-2xl border border-white/10 bg-bg-secondary/60 px-4 py-3 text-sm text-text-main">
            {templateWordLimitReached
              ? "This lesson card is over the word limit. Shorten the full lesson template before saving."
              : draft.isPublished
                ? publishReady
                  ? "This lesson card is ready to publish."
                  : "This lesson card is set to publish, but some required parts are still missing."
                : "This lesson card is in draft mode, so you can keep writing everything inside one full template."}
          </div>
          {mode === "edit" ? (
      ...
      <section className="vant-card rounded-[28px] bg-bg-secondary/35 p-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <label className="vant-card flex items-center gap-3 rounded-2xl px-4 py-3 text-text-main">
            <input name="isNewAlert" type="checkbox" defaultChecked={card?.isNewAlert ?? false} className="h-4 w-4 rounded border-white/10 bg-slate-950/80" onChange={(event) => { setDraft((current) => ({ ...current, isNewAlert: event.target.checked })); setInlineError(""); }} />
            <span>New update alert</span>
          </label>
          <label className="vant-card flex items-center gap-3 rounded-2xl px-4 py-3 text-text-main">
            <input name="isTrendingAlert" type="checkbox" defaultChecked={card?.isTrendingAlert ?? false} className="h-4 w-4 rounded border-white/10 bg-slate-950/80" onChange={(event) => { setDraft((current) => ({ ...current, isTrendingAlert: event.target.checked })); setInlineError(""); }} />
            <span>Trending update</span>
          </label>
          <label className="vant-card flex items-center gap-3 rounded-2xl px-4 py-3 text-text-main">
            <input name="isMostReported" type="checkbox" defaultChecked={card?.isMostReported ?? false} className="h-4 w-4 rounded border-white/10 bg-slate-950/80" onChange={(event) => { setDraft((current) => ({ ...current, isMostReported: event.target.checked })); setInlineError(""); }} />
      ...
      <section className="vant-card rounded-[28px] bg-bg-secondary/35 p-5">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-text-secondary">Full lesson card template</p>
        <p className="mt-3 text-sm leading-6 text-text-secondary">
          Everything below belongs to one lesson card template. Keep updating this same full card whenever the lesson grows.
        </p>

        <div className="mt-5 grid gap-6 xl:grid-cols-2">
          <label className="block xl:col-span-2">
            <span className="mb-2 block text-sm text-text-secondary">Lesson summary</span>
            <textarea
              name="description"
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={5}
              className="vant-input min-h-[140px] rounded-3xl"
            ...
            />
            {safeExampleIssue ? <p className="mt-2 text-xs text-amber-100">{safeExampleIssue}</p> : null}
          </label>
          ...
          <label className="block">
            <span className="mb-2 block text-sm text-text-secondary">Sub-lessons inside this card</span>
            <textarea
              name="howItWorks"
      ...
      <section className="vant-card rounded-[28px] bg-bg-secondary/35 p-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <label className="block"><span className="mb-2 block text-sm text-text-secondary">Current version</span><input name="currentVersion" type="number" min="1" defaultValue={card?.currentVersion ?? 1} className="vant-input" /></label>
          <label className="block"><span className="mb-2 block text-sm text-text-secondary">Re-unlock coin cost</span><input name="majorUpdateReunlockCost" type="number" min="0" defaultValue={card?.majorUpdateReunlockCost ?? ""} className="vant-input" placeholder="Optional" /></label>
          <label className="block"><span className="mb-2 block text-sm text-text-secondary">Slug override</span><input name="slug" defaultValue={card?.slug ?? ""} className="vant-input" placeholder="auto-generated-if-empty" /></label>
        </div>
      </section>
      ...
      {mode === "edit" ? (
        <section className="grid gap-4 rounded-[28px] border border-white/10 bg-bg-secondary/40 p-5 md:grid-cols-[180px_minmax(0,1fr)]">
          <label className="block">
            ...
            <select name="updateType" value={draft.updateType} onChange={(event) => { setDraft((current) => ({ ...current, updateType: event.target.value as CardUpdateType })); setInlineError(""); }} className="vant-input">
          ...
          <label className="block">
            ...
            <textarea name="changeSummary" value={draft.changeSummary} onChange={(event) => { setDraft((current) => ({ ...current, changeSummary: event.target.value })); setInlineError(""); }} rows={3} className="vant-input min-h-[100px] rounded-3xl" placeholder="Optional for routine saves. Add this when you want the update to appear in the lesson history." />
          </label>
        </section>
      ) : null}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <p className="self-center text-sm text-text-secondary">
          Publishing needs the lesson summary, safe example, quick memory rule, and all sub-lesson sections inside the same card template.
        </p>
      </div>
    </form>
  );
